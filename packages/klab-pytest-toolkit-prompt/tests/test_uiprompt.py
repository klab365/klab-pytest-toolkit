...
from klab_pytest_toolkit_prompt import UiPrompt
...
def ui_prompt():
    ...
    Returns:
        UiPrompt: A UI prompt instance ready to use
    """
    return UiPrompt()


def test_show_info_displays_and_closes(ui_prompt: UiPrompt):
...
def test_confirm_action_returns_true_on_yes(ui_prompt: UiPrompt):
...
def test_confirm_action_returns_false_on_no(ui_prompt: UiPrompt):
...
def test_confirm_action_returns_false_on_timeout(ui_prompt: UiPrompt):
    """Test that confirm_action returns False when timeout expires."""
    result = ui_prompt.confirm_action("This will timeout.", timeout=1)
    assert result is False


def test_get_user_input_returns_typed_text(ui_prompt: UiPrompt):
...
def test_get_user_input_returns_none_on_timeout(ui_prompt: UiPrompt):
